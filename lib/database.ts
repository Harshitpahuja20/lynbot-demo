...
export const userOperations = {
  async create(userData: Omit<User, "id" | "created_at" | "updated_at">) {
    ...
    const cleanUserData = {
      ...
      onboarding_complete:
        userData.onboarding_complete !== undefined
  ...
  async findAll(
    ...
    let query = supabase
      .from("users")
      .select(
        "id, email, first_name, last_name, company, role, subscription, is_active, last_login, created_at",
        { count: "exact" }
      );
...
export const prospectOperations = {
  ...
  async findByUserId(
    ...
    if (options?.search) {
      query = query.or(
        `linkedin_data->>name.ilike.%${options.search}%,linkedin_data->>company.ilike.%${options.search}%,linkedin_data->>title.ilike.%${options.search}%`
      );
    }
...
export const searchResultOperations = {
  async create(data: Omit<SearchResult, "id" | "created_at" | "updated_at">) {
    const supabase = getSupabaseAdminClient();
